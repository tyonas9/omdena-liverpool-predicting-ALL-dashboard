...
with col2:
    st.subheader("Steps")
    st.write("**Image Augmentation:** Existing images of the minority class are augmented to create new images, increasing the dataset size.")
    ...
    st.write("**Classification:** The extracted features are stored in a pandas dataframe and used to train different classifiers for image classification.")
    
st.subheader("Data Imbalance")
col3, col4 = st.columns([1,2])
with col3:
    st.write("\n")
    st.write("- The bar graph shows a significant class imbalance, with the ALL class having approximately 7200 images and the Hem class having only around 3200 images.")
    st.write("- We created new instances for the minority class using image augmentation but even then the classifiers had difficulty classifying the Hem class images.")
    st.write("- This issue affected the performance of all the models that we've trained but after extensive expermentations we were able to mitigate its affect as much as we could.")
with col4:
